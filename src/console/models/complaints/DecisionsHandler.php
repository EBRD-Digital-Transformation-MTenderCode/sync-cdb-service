...
class DecisionsHandler
{
    ...
    public static function handle($offset)
    {
        ...
        $elasticDecision = new ElasticComponent(
            Yii::$app->params['elastic_url'],
        ...
        if ($elastic_indexing) {
            ...
            $result = $elasticDecision->checkMapping();
        ...
        if (empty($offset)) {
            $offset = UpdateTime::getOffset(UpdateTime::DECISIONS);
        }
        ...
        if (!empty($data)) {
            ...
            Yii::info("Received {$receivedCount}, filtered {$filteredCount}, updated {$updatedCount} decisions. Offset: {$newOffset}", 'sync-info');
        }
        ...
        if ($offset && $offset == $newOffset) {
            ...
            UpdateTime::updateOffset(UpdateTime::DECISIONS, $newOffset);
        }
